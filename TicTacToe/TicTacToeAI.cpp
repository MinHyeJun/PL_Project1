...
{
	tttBoard = board;  // AI 내 evaluation을 계산하기 위한 가상의 게임판
...
{
	node->depth = inDepth;  // 현재 노드의 깊이를 인자로 초기화
	node->eval = 0;  // eval 값 초기화
	node->childCnt = 0;  // 자식 노드 개수 초기화
...
void TicTacToeAI::GetBestMove()
{
	...
	possible = GetPossibleMove(iList);	/*현재 가능한 수 좌표(iList에), 개수(possible에) 저장 */

	for( int i = 0; i < possible; i++ )				/* 가능한 수 만큼 Minimax 알고리즘을 사용하여 수를 계산 */
	{
		// 가능한 수의 자식 노드 중에서 가장 작은 가치를 구함
		// 이것은 가능한 수의 evaluation 값이 됨
		// 상대방은 상대방에게 가장 높은 가치를 고르기 때문에
		// 해당 순서에게 있어서 가장 작은 가치가 됨
		root->next[i] = new Node;					/* 새로운 자식 노드 생성 */
...
int TicTacToeAI::Minimize(struct treeNode* root)
{
	...
	possible = GetPossibleMove(iList); /*현재 가능한 수 좌표(iList), 개수(possible) 저장 */

	/* 가능한수 없거나, 게임이 종료되거나, 해당 컴퓨터에 설정된 레벨까지 도달할 경우
	   eval 값을 계산하여 리턴 */
	if( possible == 0 || CheckEnd() || root->depth == LAlevel )
	{
	...
	for( int i = 0; i < possible; i++ ) /* 가능한 수 만큼 Minimax 알고리즘을 사용하여 수를 계산 */
	{
		root->next[i] = new Node;					/* 자식 노드 생성 후 초기화 */
		InitNode(root->next[i], root->depth+1);  // 생성한 자식 노드의 정보를 초기화
...
/**
	함 수 : CheckSameBoard(char (*board1)[4], char (*board2)[4])
